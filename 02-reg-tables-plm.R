...
random <- plm(formula = has_union ~ pct_pop_disabledx100 + pct_pop_regoutx100 + pct_pop_woundedx100 + pct_pop_diedx100 + pct_pop_soldiersx100+ pct_pop_disabledx100*year,
              data = wctu_data_sf,
              index = c("name", "year"),
              model = "random",
...
pooling <- plm(formula = has_union ~ pct_pop_disabledx100 + pct_pop_regoutx100 + pct_pop_woundedx100 + pct_pop_diedx100 + pct_pop_soldiersx100+ pct_pop_disabledx100*year,
              data = wctu_data_sf,
              index = c("name", "year"),
              model = "pooling",
              effect = "twoways"
...
within <- plm(formula = has_union ~ pct_pop_disabledx100 + pct_pop_regoutx100 + pct_pop_woundedx100 + pct_pop_diedx100 + pct_pop_soldiersx100,
              data = wctu_data_sf,
              index = c("name"),
              model = "within",
              #random.method="walhus",
...
phtest(within, random) # fixed effects preferred because it's FE is more efficient than RE, and in the case of rejecting the null, FE is at least as consistent as RE, too. Also, I don't think county effects are independent from regressors, so random effects is inappropriate anyways.

# Main table
plm1.1i <- plm(has_union ~ pct_pop_disabledx100+pct_pop_disabledx100*year, wctu_data_sf, index = c("name", "year")) # baseline model
robust_se1i <- sqrt(diag(vcovHC(plm1.1i, type = "HC1"))) # calculate robust se

plm1.2i <- plm(has_union ~ pct_pop_disabledx100 + pct_pop_regoutx100+pct_pop_disabledx100*year, wctu_data_sf)
robust_se2i <- sqrt(diag(vcovHC(plm1.2i, type = "HC1")))

plm1.3i <- plm(has_union ~ pct_pop_disabledx100 + pct_pop_regoutx100 + pct_pop_woundedx100+pct_pop_disabledx100*year, wctu_data_sf)
robust_se3i <- sqrt(diag(vcovHC(plm1.3i, type = "HC1")))

plm1.4i <- plm(has_union ~ pct_pop_disabledx100 + pct_pop_regoutx100 + pct_pop_woundedx100 + pct_pop_diedx100+pct_pop_disabledx100*year, wctu_data_sf)
robust_se4i <- sqrt(diag(vcovHC(plm1.4i, type = "HC1")))

plm1.5i <- plm(has_union ~ pct_pop_disabledx100 + pct_pop_regoutx100 + pct_pop_woundedx100 + pct_pop_diedx100 + pct_pop_soldiersx100+pct_pop_disabledx100*year, wctu_data_sf)
robust_se5i <- sqrt(diag(vcovHC(plm1.5i, type = "HC1")))

stargazer(plm1.1i, plm1.2i, plm1.3i, plm1.4i, plm1.5i,
          df = FALSE,
...
# library(sjlabelled)
# library(sjmisc)
# plot_model(plm1.5ic,terms =  c("year [1890,1895,1896,1898]", "pct_pop_disabledx100:year [1890,1895,1896,1898]"))
# plm1.5ic$coefficients
#
library(coefplot2)
coefplot2(plm1.5i, vertical = FALSE)
...
coefplot2(plm1.5i, varnames = labels, main = "")

library(broom)
coef <- tidy(plm1.5i, conf.int = TRUE)
...
plm1.5ip <- plm1.5i # repeat last column of previous table
robust_se5ip <- robust_se5i

plm1.6ip <- plm(has_union ~ pct_pop_disabledx100 + pct_pop_regoutx100 + pct_pop_woundedx100 + pct_pop_diedx100 + pct_pop_soldiersx100+pct_pop_disabledx100*year + log_totpop, wctu_data_sf)
robust_se6ip <- sqrt(diag(vcovHC(plm1.6ip, type = "HC1")))

plm1.7ip <- plm(has_union ~ pct_pop_disabledx100 + pct_pop_regoutx100 + pct_pop_woundedx100 + pct_pop_diedx100 + pct_pop_soldiersx100+pct_pop_disabledx100*year + log_totpop + log_farmval, wctu_data_sf)
robust_se7ip <- sqrt(diag(vcovHC(plm1.7ip, type = "HC1")))

plm1.8ip <- plm(has_union ~ pct_pop_disabledx100 + pct_pop_regoutx100 + pct_pop_woundedx100 + pct_pop_diedx100 + pct_pop_soldiersx100+pct_pop_disabledx100*year + log_totpop + log_mfgcap, wctu_data_sf)
robust_se8ip <- sqrt(diag(vcovHC(plm1.8ip, type = "HC1")))

plm1.9ip <- plm(has_union ~ pct_pop_disabledx100 + pct_pop_regoutx100 + pct_pop_woundedx100 + pct_pop_diedx100 + pct_pop_soldiersx100+pct_pop_disabledx100*year + log_totpop + PresVoteTurnout1860, wctu_data_sf)
robust_se9ip <- sqrt(diag(vcovHC(plm1.9ip, type = "HC1")))

plm1.10ip <- plm(has_union ~ pct_pop_disabledx100 + pct_pop_regoutx100 + pct_pop_woundedx100 + pct_pop_diedx100 + pct_pop_soldiersx100+pct_pop_disabledx100*year + log_totpop + PctVoteRepublican1860, wctu_data_sf)
robust_se10ip <- sqrt(diag(vcovHC(plm1.10ip, type = "HC1")))

# plm1.11ip <- plm(has_union ~ pct_pop_disabledx100 + pct_pop_regoutx100 + pct_pop_woundedx100 + pct_pop_diedx100 + pct_pop_soldiersx100+pct_pop_disabledx100*year + log_totpop + PctVoteRepublican1860 + PresVoteTurnout1860 + log_mfgcap + log_farmval, wctu_data_sf)
# robust_se11ip <- sqrt(diag(vcovHC(plm1.11ip, type = "HC1")))

stargazer(plm1.5ip, plm1.6ip, plm1.7ip, plm1.8ip, plm1.9ip, plm1.10ip, #plm1.11ip,
          df = FALSE,
...
plm1.7ir <- plm(has_union ~ pct_pop_disabledx100 + pct_pop_regoutx100 + pct_pop_woundedx100 + pct_pop_diedx100 + pct_pop_soldiersx100+pct_pop_disabledx100*year + log_totpop, wctu_data_sf %>% filter(year %in% c(1882,1890,1895)))
robust_se7ir <- sqrt(diag(vcovHC(plm1.7ir, type = "HC1")))

plm1.8ir <- plm(has_union ~ pct_pop_disabledx100 + pct_pop_regoutx100 + pct_pop_woundedx100 + pct_pop_diedx100 + pct_pop_soldiersx100+pct_pop_disabledx100*year + log_totpop, wctu_correct_sf %>% filter(year %in% c(1882,1890,1895)))
robust_se8ir <- sqrt(diag(vcovHC(plm1.8ir, type = "HC1")))

plm1.9ir <- plm(has_union ~ pct_pop_disabledx100 + pct_pop_regoutx100 + pct_pop_woundedx100 + pct_pop_diedx100 + pct_pop_soldiersx100+pct_pop_disabledx100*year + log_totpop, hazard_sf %>% filter(year %in% c(1882,1890,1895)))
robust_se9ir <- sqrt(diag(vcovHC(plm1.9ir, type = "HC1")))

stargazer(plm1.6ir, plm1.7ir, plm1.8ir, plm1.9ir,
          df = FALSE,
